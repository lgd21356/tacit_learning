...
from dog_rl_deploy.lcm_types.logger_lcmt import logger_lcmt
import lcm
        ...
        try:
            for i in range(max_steps):
                ...
                for agent_name in self.agents.keys():
                    ...
                    if self.agents[self.control_agent_name].se.auto_cmd:
                        lcm_logger.time=i * self.agents[self.control_agent_name].dt
                        lcm_logger.joint_torques=self.agents[self.control_agent_name].se.get_tau_est()
                        lcm_logger.joint_positions=self.agents[self.control_agent_name].se.get_dof_pos()
                        lcm_logger.joint_velocities=self.agents[self.control_agent_name].se.get_dof_vel()
                        lcm_logger.contact_forces=self.agents[self.control_agent_name].se.get_ori_contact_state()
                        lcm_logger.output_torques=self.agents[self.control_agent_name].torques
                        lcm_logger.tacit_torques=self.agents[self.control_agent_name].tacit_torques
                        lc.publish("lcm_logging", lcm_logger.encode())
