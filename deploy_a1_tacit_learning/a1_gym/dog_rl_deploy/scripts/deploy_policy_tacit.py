...
from dog_rl_deploy.envs.lcm_agent_tacit import LCMAgentTacit
...
def load_and_run_policy(label, experiment_name, probe_policy_label=None, max_vel=1.0, max_yaw_vel=1.0, max_vel_probe=1.0):
    ...
    control_dt = 0.02
    command_profile = AutoRCControllerProfile(dt=control_dt, state_estimator=se, x_scale=max_vel, y_scale=0.6, yaw_scale=max_yaw_vel, probe_vel_multiplier=(max_vel_probe / max_vel))

    hardware_agent = LCMAgentTacit(cfg, se, command_profile)
...
if __name__ == '__main__':
    label = "gait-conditioned-agility/2024-01-08/train"

    probe_policy_label = None

    experiment_name = "example_experiment"

    load_and_run_policy(label, experiment_name=experiment_name, probe_policy_label=probe_policy_label, max_vel=2.5, max_yaw_vel=5.0, max_vel_probe=1.0)
