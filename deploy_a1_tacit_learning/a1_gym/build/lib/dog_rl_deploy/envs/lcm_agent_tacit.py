...
class LCMAgentTacit():
    def __init__(self, cfg, se, command_profile):
        ...
        self.current_mode=self.se.mode
        ...
        self.tacit_torques = np.zeros(12)
    ...
    def publish_action(self, action, hard_reset=False):
        ...
        if self.se.cmd_tacit:
            self.tacit_torques+=0.0002*self.torques
            self.tacit_torques[[0,3,6,9]]=np.clip(self.tacit_torques[[0,3,6,9]], -1, 1)
            # clamp the tacit torques
            self.tacit_torques[[1,2,4,5,7,8,10,11]]=np.clip(self.tacit_torques[[1,2,4,5,7,8,10,11]], -1, 1)
            # self.torques+=self.tacit_torques
        if self.current_mode != self.se.mode:
            self.tacit_torques[:]=0
            self.current_mode= self.se.mode
        #self.tacit_torques[[0,3,6,9]]=0
        ...
        command_for_robot.tau_ff = self.tacit_torques
