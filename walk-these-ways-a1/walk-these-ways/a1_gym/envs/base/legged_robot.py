...
class LeggedRobot(BaseTask):
    ...
    def _resample_commands(self, env_ids):
        ...
        if self.cfg.commands.num_commands > 5:
            if self.cfg.commands.gaitwise_curricula:
                for i, (category, env_ids_in_category) in enumerate(zip(self.category_names, category_env_ids)):
                    if category == "pronk":  # pronking
                        ...
                        self.commands[env_ids_in_category, 5] = self.commands[env_ids_in_category, 5] / 2 + 0.25
                        self.commands[env_ids_in_category, 6] = self.commands[env_ids_in_category, 6] / 2 + 0.125
                        self.commands[env_ids_in_category, 7] = 0
                    elif category == "gallop":
                        self.commands[env_ids_in_category, 5] = self.commands[env_ids_in_category, 5] / 2 + 0.125
                        self.commands[env_ids_in_category, 6] = 0
                        self.commands[env_ids_in_category, 7] = 0

            elif self.cfg.commands.exclusive_phase_offset:
                random_env_floats = torch.rand(len(env_ids), device=self.device)
                trotting_envs = env_ids[random_env_floats < 0.2]
                pacing_envs = env_ids[torch.logical_and(0.2 <= random_env_floats, random_env_floats < 0.4)]

                bounding_envs = env_ids[torch.logical_and(0.4 <= random_env_floats, random_env_floats < 0.6)]
                walking_envs = env_ids[torch.logical_and(0.6 <= random_env_floats, random_env_floats < 0.8)]
                gallop_envs = env_ids[0.8 <= random_env_floats]
                ...
                self.commands[walking_envs, 7] = 0
                self.commands[gallop_envs, 6] = 0
                self.commands[gallop_envs, 7] = 0

            elif self.cfg.commands.balance_gait_distribution:
                random_env_floats = torch.rand(len(env_ids), device=self.device)
                pronking_envs = env_ids[random_env_floats <= 0.17]
                trotting_envs = env_ids[torch.logical_and(0.17 <= random_env_floats, random_env_floats < 0.34)]
                pacing_envs = env_ids[torch.logical_and(0.34 <= random_env_floats, random_env_floats < 0.51)]
                bounding_envs = env_ids[torch.logical_and(0.51 <= random_env_floats, random_env_floats < 0.68)]
                walking_envs  = env_ids[torch.logical_and(0.68 <= random_env_floats, random_env_floats < 0.85)]
                gallop_envs = env_ids[0.85 <= random_env_floats]
                ...
                self.commands[walking_envs, 5] = self.commands[walking_envs, 5] / 2 + 0.25
                self.commands[walking_envs, 6] = self.commands[walking_envs, 6] / 2 + 0.125
                self.commands[walking_envs, 7] = 0
                self.commands[gallop_envs, 5] = self.commands[gallop_envs, 5] / 2 + 0.125
                self.commands[gallop_envs, 6] = 0
                self.commands[gallop_envs, 7] = 0
            ...
            if self.cfg.commands.binary_phases:
                self.commands[env_ids, 5] = (torch.round(4 * self.commands[env_ids, 5])) / 4.0 % 1
                self.commands[env_ids, 6] = (torch.round(4 * self.commands[env_ids, 5])) / 4.0 % 1
                self.commands[env_ids, 7] = (torch.round(4 * self.commands[env_ids, 5])) / 4.0 % 1
    ...
    def custom_round(self, value):
        # Multiply by 4 to scale
        scaled_value = 4 * value
        # Floor the scaled value to get the lower bound, then convert back to float
        floored_value = torch.floor(scaled_value).float()
        # Cap the value at 2 (corresponding to 0.5) to avoid going to 0.75, ensuring all tensors are floats
        capped_value = torch.where(floored_value < 2, floored_value, torch.tensor(2.0))
        # Scale back
        return capped_value / 4
    ...
    def _compute_torques(self, actions):
        ...
        if hasattr(self.cfg.control, 'tacit_gain'):
            self.tacit_torques+= self.cfg.control.tacit_gain * torques
            torques += self.tacit_torques
    ...
    def _init_buffers(self):
        ...
        self.torques = torch.zeros(self.num_envs, self.num_dof, dtype=torch.float, device=self.device,
                                   requires_grad=False)
        self.tacit_torques = torch.zeros(self.num_envs, self.num_dof, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _init_command_distribution(self, env_ids):
        ...
        if self.cfg.commands.gaitwise_curricula:
            self.category_names = ['pronk', 'trot', 'pace', 'bound', 'walk', "gallop"]
